...
create_release() {
  pushd korifi
  {
    kubectl kustomize api/config/overlays/pr-e2e | kbld -f "$KBLD_CONFIG_DIR/korifi-api-kbld.yml" -f "$RELEASE_ARTIFACTS_DIR/korifi-api.yml"
    kubectl kustomize controllers/config/overlays/pr-e2e | kbld -f "$KBLD_CONFIG_DIR/korifi-controllers-kbld.yml" -f "$RELEASE_ARTIFACTS_DIR/korifi-controllers.yml"
  }
  popd
