...
team=$(gh repo view cloudfoundry/korifi-controllers --json assignableUsers --jq '.assignableUsers[].login')

for user in $team; do
  if [[ "$user" == "$PR_AUTHOR" ]]; then
    echo Allowing e2e run for team member "$user"
    gh pr --repo cloudfoundry/korifi-controllers edit $PR_NUMBER --add-label "e2e-allowed"
  fi
done
